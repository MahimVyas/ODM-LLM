...
export default function Home() {
  ...
  const confirmInitialize = async () => {
    ...
    try {
      ...
      alert(`Initialization Failed: ${error.message || "Your device may not support WebGPU or ran out of memory."}`);
      engineWorkerRef.current?.terminate();
  ...
  return (
    // 🚀 FIX 1: Changed `h-screen` to `h-[100dvh]` to account for mobile browsers
    <div className="flex h-[100dvh] bg-[#212121] text-gray-100 font-sans selection:bg-gray-700 overflow-hidden relative">
...
{/* --- HEADER --- */}
        ...
        {/* --- MAIN CHAT AREA --- */}
        {/* 🚀 FIX 3: Removed huge bottom padding, letting the flexbox handle spacing natively */}
        ...
        {/* --- BOTTOM INPUT BAR --- */}
        {/* 🚀 FIX 4: Footer is no longer "absolute", it strictly sits at the bottom of the flex column */}
        <footer className="shrink-0 w-full z-10 pb-4 sm:pb-6 pt-2 px-4 sm:px-6 bg-[#212121]">
          <div className="max-w-3xl mx-auto relative">
            
            {/* 🚀 Adjusted the image preview anchor to sit correctly over the new footer */}
            {selectedImage && (
