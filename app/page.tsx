...
const MessageItem = memo(({ role, content }: { role: string; content: string }) => (
  ...
  >
    <div
      className={`px-5 py-3 rounded-2xl max-w-[85%] shadow-sm ${role === "user" ? "bg-gray-100 text-gray-950 rounded-br-sm" : "bg-gray-900 text-gray-100 border border-gray-800 rounded-bl-sm"}`}
    >
      {content}
    </div>
  </div>
...
export default function Home() {
  ...
  const [activeModelId, setActiveModelId] = useState(AVAILABLE_MODELS[0].id);
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ensureVisionWorker(); // Ensure worker exists when user interacts
  ...
  const handleSend = async () => {
    ...
    ensureVisionWorker(); // Ensure vision worker is ready before using it
      ...
      <div className="flex-1 flex flex-col relative min-w-0">
        ...
        <header className="absolute top-0 w-full z-20 backdrop-blur-md bg-gray-950/70 border-b border-gray-800">
          <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
            ...
            {/* Right side: Model Selector & Load Button */}
        ...
        {showInitConfirm && (
          <div className="absolute inset-0 z-[100] flex items-center justify-center p-4 bg-gray-950/80 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="w-full max-w-md bg-gray-900 border border-gray-800 rounded-3xl shadow-2xl p-8 animate-in slide-in-from-bottom-4 duration-300">
              ...
              <p className="text-[10px] text-center text-gray-500 mt-4 leading-relaxed px-4">
                Initial download may take a few minutes depending on your connection. Subsequent loads will be near-instant.
              </p>
            </div>
          </div>
        )}
            ...
            <>
              ...
              {(streamingContent || visionStatus) && (
                <div className="flex justify-start">
                  <div className="px-5 py-3 rounded-2xl max-w-[85%] shadow-sm bg-gray-900 text-gray-100 border border-gray-800 rounded-bl-sm">
                    {visionStatus && (
        ...
        {/* --- Footer Input Area --- */}
        <footer className="absolute bottom-0 w-full z-10 pb-6 pt-4 px-4 bg-gradient-to-t from-gray-950 via-gray-950 to-transparent">
